<template>
  <div class="enhanced-display-view display-a" :class="{ 'full-screen': isFullScreen }">
    <!-- 连接状态 -->
    <div class="connection-status" :class="connectionClass">
      <el-icon><Connection /></el-icon>
      <span>{{ connectionText }}</span>
      <span class="screen-label">A屏</span>
    </div>
...
const isActiveScreen = computed(() => {
  return timerStore.isActiveScreen('A')
})
...
const displayRemaining = computed(() => {
  if (timerState.abMode === 'alternate') {
    ...
    if (timerStore.isActiveScreen('A') && timerState.screenAStatus === 'running') {
...
const screenStatus = computed(() => {
  // 判断当前屏幕状态
  if (timerState.abMode === 'alternate') {
    return timerState.screenAStatus || 'paused'
  }
...
watch(() => timerState.screenAStatus, (newStatus, oldStatus) => {
  if (newStatus && newStatus !== oldStatus) {
    logService.debug(`A屏状态变化: ${oldStatus} -> ${newStatus}`)
  }
})
